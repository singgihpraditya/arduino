...
boolean isOkToForward = true;

void setup() {
  Serial.begin(57600);
//  error = ps2x.config_gamepad(13, 11, 10, 12, true, true); //GamePad(clock, command, attention, data, Pressures?, Rumble?)
  error = ps2x.config_gamepad(clockPin, commandPin, attentionPin, dataPin, true, true); //GamePad(clock, command, attention, data, Pressures?, Rumble?)
...
void loop() {
  ...
  if (ps2x.Button(PSB_PAD_UP) && isOkToForward) {
